...
import {gptPrompt} from "../util/gptPrompt";
...
export const RecordSequenceDetails = (props: MProps) => {
    ...
    let variables = [];
    const [llmVariable, setLlmVariable] = useState<any>([]);

    const llmQuery = async () => {
        const prompt = 'From the User query \''+props.searchKeyword.toString()+'\', extract the values for the following variable: '+llmVariable.toString();
        const llmResponse = await gptPrompt.getResponseFromGPT(prompt);
        console.log(llmResponse);
    }

    useEffect(()=>{
        if(llmVariable.length > 0){
            llmQuery();
        }
    },[llmVariable])
    ...
    const processMetaTags = () => {
        for (let index = 0; index < selectedRecordingDetails?.userclicknodesSet.length; index++) {
            const item = selectedRecordingDetails?.userclicknodesSet[index];
            const originalNode = getObjData(item?.objectdata);
            //adding llm inputs from abhishek
            if (originalNode.meta.hasOwnProperty('customMetaData') && originalNode.meta.customMetaData) {
                let inputString: string = originalNode.meta.customMetaData;
                const match = inputString.match(/input:(.*)/);
                // add regular expression to match input: to a string
                if (match) {
                    variables.push(match[1].trim());
                }
                if (index + 1 === selectedRecordingDetails?.userclicknodesSet.length) {
                    console.log(index);
                    setLlmVariable(variables);
                }
            }
        }
    }
                ...
                />*/}
                    ...
                    <Col span={8}>
                        {(selectedRecordingDetails.usersessionid === userId) &&
                            <Popconfirm title={translate('deleteRecording')} onConfirm={removeRecording}
                                        className="uda_exclude">
